...
2. **Set up a virtual environment and install packages**:
   The required packages and their exact version numbers are specified in [requirements.txt](requirements.txt). Install them via:
   ```bash
   python3 -m venv .venv
...
```
*Outputs are saved under `results/full/`. (Note: To evaluate impossible/external datasets, follow instructions in [DATA_SETUP.md](DATA_SETUP.md)).*
...
*   **Train Symmetric Polarity DPO**:
    ...
    python src/rl/sym_mpo_train.py \
        ...
        --max_steps 1000
    ```
    *Training parameters are defined in [configs/symmetric_dpo.yaml](configs/symmetric_dpo.yaml).*
